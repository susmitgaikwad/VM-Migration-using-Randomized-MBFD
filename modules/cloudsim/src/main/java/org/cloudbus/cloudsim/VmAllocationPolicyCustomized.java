...
public class VmAllocationPolicyCustomized{
	...
	private static ArrayList<Integer> temp_maptemp=new ArrayList<Integer>();
	...
	public ArrayList<Integer> allocationForMigration(List<Host> hostList, List<Vm> vmList){
		...
		if (flag==false)
		{
			...
			for (Vm vm : vmList) {
				int flip=1;
				if (!(migrationVmList.isEmpty())) {
					for (Vm temporary_vm: migrationVmList) {
						if (temporary_vm.getId()==vm.getId()) {
							flip=0;
						}
					}
				}
				if (host.getId()==vm.getHost().getId() && flip==1) {
			...
			if (choice==1) {
				...
				for (Host host : nothotspotList) {
					...
					if (temp_maptemp.size()!=0)
					{
					Vm vm_already_allocated=null;
					for (int a=1;a<temp_maptemp.size();a=a+2) {
						if (temp_maptemp.get(a)==host.getId()) {
							for (Vm temp_vm:vmList) {
								if (temp_vm.getId()==temp_maptemp.get(a-1)) {
									vm_already_allocated=temp_vm;
									total= (long) (total+vm_already_allocated.getTotalUtilizationOfCpuMips(CloudSim.clock())+vm_already_allocated.getCurrentAllocatedRam()+vm_already_allocated.getCurrentAllocatedBw());	
								}
							}
						}
			...
			for (Host host : hotspotList) {
				...
				if (choice==1) {
					...
					for (Host targethost:nothotspotList) {
						...
						Vm vm_already_allocated=null;
						for (int a=1;a<temp_maptemp.size();a=a+2) {
							if (temp_maptemp.get(a)==targethost.getId()) {
								for (Vm temp_vm:vmList) {
									if (temp_vm.getId()==temp_maptemp.get(a-1)) {
										vm_already_allocated=temp_vm;
										targethost_cpu=targethost_cpu+vm_already_allocated.getTotalUtilizationOfCpuMips(CloudSim.clock());
										targethost_ram=targethost_ram+vm_already_allocated.getCurrentAllocatedRam();
										targethost_bw=targethost_bw+vm_already_allocated.getCurrentAllocatedBw();	
									}
								}
							}
						...
						if ((targethost_cpu+targetmips)<targethost_max_cpu && (targethost_cpu+targetmips)>targethost_min_cpu) {
							...
							totalutilizationdifference.add(minimum_difference);
						}
					}
				...
				temp_maptemp.add(vm.getId());
				temp_maptemp.add(nothotspotList.get(targethostindex).getId());
			...
			for (Vm vm : vmList) {
				int flip=1;
				for (Vm migrating_vm:migrationVmList) {
					if (migrating_vm.getId()==vm.getId()) {
					flip=0;
					}
				}
				if (host.getId()==vm.getHost().getId() && flip==1) {
			...
			if (mips>max_util_cpu || ram>max_util_mem || bw>max_util_bw) {
				System.out.println("IIIIIMMMMMMMMPPPPPPPPPPP");
				
				flag_for_host=1;
			}
			else if (mips<min_util_cpu || ram<min_util_mem || bw<min_util_bw) {
				System.out.println("PPPPPPPMMMMMMMMMIIIIIIII");
				flag_for_host=1;
			}
			...
			double avg_balance_degree=0.0;
			double final_utilization=0.0;
			List<Double> final_total_utilization=new ArrayList<Double>();
			for (Host host:hostList){
			...
			for (Vm vm : vmList) {
				...
				if (host.getId()==vm.getHost().getId() && flip==1) {
					...
					ram=ram+vm.getCurrentAllocatedRam();
					bw=bw+vm.getCurrentAllocatedBw();
					final_utilization=final_utilization+mips+ram+bw;
				...
				for (int a=1;a<temp_maptemp.size();a=a+2) {
					if (temp_maptemp.get(a)==host.getId()) {
						for (Vm temp_vm:vmList) {
							if (temp_vm.getId()==temp_maptemp.get(a-1)) {
								vm_already_allocated=temp_vm;
								final_utilization=final_utilization+vm_already_allocated.getTotalUtilizationOfCpuMips(CloudSim.clock());
								final_utilization=final_utilization+vm_already_allocated.getCurrentAllocatedRam();
								final_utilization=final_utilization+vm_already_allocated.getCurrentAllocatedBw();	
							}
						}
					}
			...
			final_total_utilization.add(final_utilization);	
			}
			double total_utilization=0.0;
			for(int s=0;s<hostList.size();s++) {
				total_utilization=total_utilization+final_total_utilization.get(s);
			}
			total_utilization=total_utilization/hostList.size();
			for (int k=0;k<hostList.size();k++) {
				avg_balance_degree=avg_balance_degree+((total_utilization-final_total_utilization.get(k))*(total_utilization-final_total_utilization.get(k)));
				
			}
			avg_balance_degree=avg_balance_degree/hostList.size();
			avg_balance_degree=Math.sqrt(avg_balance_degree);
			System.out.println("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$");
			System.out.println(avg_balance_degree);
			System.out.println("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$");
		}
		else if(flag==true)
		{
...
public int random_fit(Vm vm, List<Vm> vmList) {
	int host_index=-1;
	double targetmips=vm.getTotalUtilizationOfCpuMips(CloudSim.clock());;
	int targetram=vm.getCurrentAllocatedRam();
	long targetbw=vm.getCurrentAllocatedBw();
