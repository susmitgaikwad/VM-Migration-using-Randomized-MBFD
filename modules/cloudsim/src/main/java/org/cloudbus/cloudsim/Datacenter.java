...
import org.cloudbus.cloudsim.VmAllocationPolicyCustomized;
...
public class Datacenter extends SimEntity {
	...
	private VmAllocationPolicyCustomized vmAllocationPolicyCustomized;
	/**
	...
	protected void processVmMigrate(SimEvent ev, boolean ack) {
		...
		if (vm==null)
		{
				System.out.println("kjdfkjdfkjskdj");
		}
	...
	protected void processCloudletSubmit(SimEvent ev, boolean ack) {
		...
		try {
			...
			int original_host_id=host.getId();
			Vm vm = host.getVm(vmId, userId);
			List<Host> hList = getVmAllocationPolicy().getHostList();
			...
			for (int i=0;i<getVmAllocationPolicy().getHostList().size();i++)
			{
				List<Vm> vmListTemp=getVmAllocationPolicy().getHostList().get(i).getVmList();
				for (int j=0;j<vmListTemp.size();j++)
				{
					vmList.add(vmListTemp.get(j));
				}
			}
			
			List<Host> hostList=new ArrayList<Host>();
			hostList=getVmAllocationPolicy().getHostList();
			
			
			System.out.println("Size of host listin datacenter is: "+hostList.size());
			System.out.println("Size of vm listin datacenter is: "+vmList.size());
			
			if(cl.getVmId()==vmList.size()-1)
			{
			int countofmigrations=0;
			VmAllocationPolicyCustomized vmAllocationPolicyCustomized=new VmAllocationPolicyCustomized();
			ArrayList<Integer> maptemp=new ArrayList<Integer>();
			maptemp=vmAllocationPolicyCustomized.allocationForMigration(hostList,vmList);
			int host_id=0;
			int vm_id=0;
			...
			for (int i=0;i<maptemp.size();i=i+2){
				...
				ma.put("Host",getVmAllocationPolicy().getHostList().get(host_id));
			
				//getVmAllocationPolicy().get
				ma.put("VM",vmList.get(vm_id));
				double delay=1.0;
				System.out.println("Krishna hai! radha hai!");
				send(this.getId(),delay,CloudSimTags.VM_MIGRATE,ma);
				countofmigrations++;
			}
			System.out.println("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&");
			System.out.println(countofmigrations);
			System.out.println("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&");
			}
		} catch (ClassCastException c) {
